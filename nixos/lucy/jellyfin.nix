...
let
  ...
  writeJsonFile = filename: json: pkgs.writeText filename (toJSON json);
  ...
  escapeURL =
    ...
    if v == true then
      ...
      lib.strings.escapeURL v;
  formatUrlParams =
    attrs:
    concatStringsSep "&" (mapAttrsToList (name: value: "${escapeURL name}=${escapeURL value}") attrs);
in
{
  options.services.jellyfin =
            ...
            fetchers =
              ...
              mkOption {
                ...
                default =
                  ...
                  else if config.type == "movies" then
                    {
                      Movie = {
                        metadata = [
                          "TheMovieDb"
                  ...
                  else if config.type == "tvshows" then
                    {
                      Series = {
                        metadata = [
                          "TheMovieDb"
                  ...
                  else if config.type == "homevideos" then
                    {
                      Video = {
                        metadata = [ ];
                        images = [
    ...
    in
    {
      ...
      forceReconfigure = mkEnableOption "deleting all Jellyfin config and starting again";
    };
  ...
  config =
    let
      configScript =
        let
          wizardConfigFile = writeJsonFile "wizard-config" {
            ...
            PreferredMetadataLanguage = cfg.localisation.preferredMetadataLanguage;
          };
        ...
        pkgs.mypkgs.writeCheckedShellScript {
          ...
          text =
            let
              ...
              device = if opt.virtualHost.fqdn.isDefined then cfg.virtualHost.fqdn else config.networking.fqdn;
              version = "0";
            in
            ''
      ...
      virtualHostSecureConfig =
        mkIf
          ((cfg.virtualHost.enableACME || cfg.virtualHost.forceSecureConnections) && cfg.virtualHost.enable)
          {
      ...
      reconfigureConfig = {
        ...
        systemd.services.jellyfin.restartTriggers = mkIf cfg.forceReconfigure [
