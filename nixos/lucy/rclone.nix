...
let
  ...
  rcloneMountSubmodule =
    { config, ... }:
    {
      ...
      config = {
        unitName = lib.mkDefault "rclone-mount@${pkgs.mylib.escapeSystemdPath config.where}";
        ...
        unitConfig = lib.recursiveUpdate {
...
in
{
  ...
  config = lib.mkIf (cfg.mounts != [ ]) {
    ...
    systemd.services = lib.mergeAttrsList (map (u: { "${u.unitName}" = u.unitConfig; }) cfg.mounts);
  };
}
