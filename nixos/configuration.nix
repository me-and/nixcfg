...
in {
  imports =
    ...
    # hardware-configuration.nix is expected to be missing on WSL.  And, for
    # flexibility, we allow both hardware-configuration.nix and
    # local-config.nix to be adjacent to either of the configuration.nix files.
    ++ filesIfExtant [
