{
  ...
  tmuxCommands = writeTextFile {
    name = "tmux-taskloop-script";
    text = ''
      # Start the session.  We'll have a single session called "taskloop" with
      # a single window with the same name.
      #
      ...
      # Don't do that here, because that prevents the session knowing the
      # correct size for its windows, and passing that information through
      # seems remarkably complicated.
      #
      ...
      # Create a new pane below the previous pane, with the waitingfor
      # taskloop.
      ...
      # Trigger the two taskloop panes to actually load, now their size is
      # fixed.
      send-keys -t %1 C-m
      send-keys -t %2 C-m
    '';
