# Commands for using operon on multiple files at once.
{ quodlibet-without-gst-plugins, writeCheckedShellApplication }:
writeCheckedShellApplication {
  name = "operons";
  runtimeInputs = [ quodlibet-without-gst-plugins ];
  excludeShellChecks = [
    "SC2016" # I know about $ non-expansion within single quotes
  ...
  text = ''
    ...
    operons_onetag () {
        ...
        if [[ "$file_name" && "$escape" ]]; then
            local commands=(-print0)
            _operons_act commands "$@" | while read -r -d "" f; do
                output="$(operon print -p "<''${tag}>" "$f")"
                printf '%q: %q\n' "$f" "$output"
            done
        else
            if [[ "$file_name" ]]; then
    ...
    operons_store () {
        ...
        target=()
        local commands=(-print0)
        _operons_act commands "$@" | while read -rd "" f; do
            target+=("$f")
        done
    }
