...
let
  ...
  systemChannelDirContents = readDir /nix/var/nix/profiles/per-user/root/channels;
  expectedContents = { "manifest.nix" = "symlink"; } // (mapAttrs (k: v: "symlink") config.nix.channels);
in {
  options = {
