...
let
  ...
  primaryEmailAccount = lib.lists.findSingle (acc: acc.primary && acc.enable) null null (
...
in
{
  ...
  home.activation = lib.mkIf (primaryEmailAccount != null) {
    dotForward = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
