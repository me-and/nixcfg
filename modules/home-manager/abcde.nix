# TODO This is the full contents of the abcde.conf template from the upstream
# source, less bits I've already integrated into the Nix module.
#
...

{
  ...
  inherit (lib.strings) concatStringsSep toShellVars toUpper;
  inherit (lib.attrsets) concatMapAttrs filterAttrs mapAttrs optionalAttrs;
  inherit (lib.lists) any;

  cfg = config.programs.abcde;

  encoders = let
    anyInFormatList = any (v: builtins.elem v cfg.output.format);
  in {
    opusenc = {
      enableOptions = {
        default = builtins.elem "opus" cfg.output.format;
      };
    ...
    lame = {
      enableOptions = {
        default = builtins.elem "mp3" cfg.output.format;
      };
      optionsDescription = ''
        Look at
        http://wiki.hydrogenaudio.org/index.php?title=LAME#Recommended_encoder_settings
    ...
    oggenc = {
      enableOptions = {
        default = anyInFormatList ["ogg" "vorbis"];
      };
      packageName = "vorbis-tools";
    };

    flac = {
      enableOptions = {default = builtins.elem "flac" cfg.output.format;};
      optionsDescription = ''
    ...
    fdkaac = {
      ...
      '';
      optionsOptions = {default = "--bitrate 192k";};
    };
    ...
    ffmpeg = {
      enableOptions = {
        default = anyInFormatList ["mka" "aiff"];
      };
      optionsDescription = ''
        FFmpeg can be used for several audio codecs, as well as being the
        default encoder for the Matroska container mka:

        1.  Encoding to WavPack.  Consider setting the following with a
            compression_level between 0-8:

                programs.abcde.encoders.ffmpeg.options = "-c:a wavpack -compression_level 6"

        2.  Encoding to ALAC.  Consider using the following:

                programs.abcde.encoders.ffmpeg.options = "-c:a alac"

        3.  Encoding to mp2.  Consider using the following:

                programs.abcde.encoders.ffmpeg.options = "-c:a mp2 -b:a 320k"
      '';
      packageName = "ffmpeg-headless";
      optionsVarName = "FFMPEGENCOPTS";
    };
  };
in {
  options.programs.abcde = {
    enable = mkEnableOption "abcde, A Better CD Encoder";
    package = mkPackageOption pkgs "abcde" {};
    ...
    in lib.mkOption {
      description = ''
        ...
        "default", without having to repeat the default ones:
             ...
             programs.abcde.actions = ["default" "playlist"];

        The default action list (referenced as "default") is: cddb, read,
        encode, tag, move, clean.
      '';
      type = types.listOf (types.enum validActions);
      default = ["default"];
      example = ["default" "playlist"];
    };
    ...
    output = {
      ...
      in lib.mkOption {
        description = ''
          The output file formats.  Currently recognised and supported are
          "${concatStringsSep "\", \"" validFormats}".
        '';
        type = types.listOf (types.enum validFormats);
        default = ["ogg"];
        example = ["opus" "flac"];
      };
    };
    ...
    encoders = let
      makeEncoderOptions = name: {
        ...
        enable = mkOption ({
          description = ''
            Whether to enable using the ${name} encoder";
          '';
          type = lib.types.bool;
          default = false;
          example = true;
        ...
        options = mkOption ({
          description = "Encoder options to use." + lib.optionalString (optionsDescription != null) ''
        ...
        package = mkPackageOption pkgs packageName packageOptionAttrs;
        executable = lib.mkOption {
          description = "Path to the encoder executable";
          type = lib.types.path;
          default =
            if executable == null
            then "${cfg.encoders."${name}".package}/bin/${executableName}"
            else executable;
        };
      };
    in mapAttrs makeEncoderOptions encoders;

    configDefs = mkOption {
      # TODO Make this a submodule that allows defining additional values while
      # treating the common values as defaults, rather than having any
      # definition of this value override the defaults.
      description = ''
        Definitions to add to the .abcde.conf file.  These will be escaped; if
        you want values that will be calculated dynamically, use
        programs.abcde.extraConfig.
      '';
      type = types.attrsOf (types.nullOr (types.oneOf [types.str types.int types.bool]));
      default = let
        makeEncoderConfig = name: {
          varName ? toUpper name,
          executableVarName ? varName,
          optionsVarName ? "${varName}OPTS",
          extraDefs ? {},
          ...
        }: let
          encoderCfg = cfg.encoders."${name}";
        in optionalAttrs encoderCfg.enable ({
          "${executableVarName}" = encoderCfg.executable;
          "${optionsVarName}" = encoderCfg.options;
        } // extraDefs);
      in {
        ACTIONS = concatStringsSep "," cfg.actions;
        OUTPUTTYPE = concatStringsSep "," cfg.output.format;
      }
      // concatMapAttrs makeEncoderConfig encoders;
    };

    extraConfig = mkOption {
      description = ''
        Configuration to add to the .abcde.conf configuration file.
      '';
      type = types.lines;
      default = "";
    };
    fullConfig = mkOption {
      description = ''
        ...
        extraConfig to add more configuration that isn't directly supported by
        the programs.abcde module.
      '';
      type = types.lines;
      default = let
        nonNullDefs = filterAttrs (n: v: v != null) cfg.configDefs;
        parsedDefs = mapAttrs (n: v:
          if v == true
          then "y"
          else if v == false
          then "n"
  ...
  config = mkIf cfg.enable {
    home.file.".abcde.conf".text = cfg.fullConfig;
